...
class EditBookingPage extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, ref) {
    ...
    final id = useState("");
    // ref.listen(EditBookingStateProvider, (previous, next) {
    //   next!.detailBookings!.map((e) => arrSelectProduct.value.add(e.product!)).toList();
    // });
    useEffect(() {
      ref.read(PelangganListProvider.notifier).list();
      ref.read(ProductListProvider.notifier).list();
      dropdownValuePelanggan.value =
          ref.read(EditBookingStateProvider.notifier).state!.pelanggan!;
      ref
          .read(EditBookingStateProvider.notifier)
          .state!
          .detailBookings!
          .map((e) => arrSelectProduct.value.add(e.product!))
          .toList();
      id.value =
          ref.read(EditBookingStateProvider.notifier).state!.id!.toString();
      selectedDate.value = DateTime.parse(
          ref.read(EditBookingStateProvider.notifier).state!.tglBooking!);
      return;
    }, []);
    ...
    return Scaffold(
      body: SafeArea(
        ...
        child: Column(
          ...
          children: [
            HeaderWidget(title: "Edit Booking"),
            Container(
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: DropdownButton<Pelanggan>(
                      ...
                      isExpanded: true,
                      hint: Text(dropdownValuePelanggan.value!.name != null
                      ...
                      onPressed: () {
                        ...
                        if (arrSelectProduct.value.isEmpty) {
                          ...
                          List<DetailBookings2> dboo = r
                              ...
                              .toList();
                          ref.watch(BookingProvider.notifier).edit(
                              b: BookingRequest(
                                  detailBookings: dboo,
                                  idPelanggan: dropdownValuePelanggan.value!.id,
                                  tglBooking: selectedDate.value.toString()),
                              id: id.value, tgl: DateFormat('yyyy-MM-dd').format(selectedDate.value));
                        }
                      },
                      child: Container(
                        decoration: BoxDecoration(
